// server.js (ESM) — Kie.ai Veo-3 + Runway
// Requires: express, cors, axios, dotenv
// ENV: KIE_KEY (required), KIE_API_PREFIX (optional, default https://api.kie.ai/api/v1)
//      PORT (optional)
...
app.get("/health", (req, res) => {
  res.json({
    ...
    env: { kie_key_present: Boolean(KEY), api_prefix: API }
  });
});

app.get("/", (req, res) => {
  res.json({ status: "✅ Kie.ai backend running", version: "1.0.0", time: new Date().toISOString() });
});

// ------------- helpers
const ALLOWED_RATIOS = new Set(["16:9", "9:16", "1:1", "4:3", "3:4"]);
function clampDur(d){ const n=Number(d); if(!Number.isFinite(n)) return 8; return Math.max(1, Math.min(8, Math.round(n*10)/10)); }

function sanitize(body={}, {withAudioDefault=true} = {}) {
...
// ------------- Runway via Kie (great for B-roll/bridges)
app.post("/generate-runway", async (req, res) => {
  try {
    const p = sanitize(req.body);
    const sub = await kiePost("/runway/generate", p);   // same payload shape
    const jobId = sub.id || sub.job_id;
    if (!jobId) return res.status(502).json({ success:false, error:"No job id from Kie", raw: sub });
